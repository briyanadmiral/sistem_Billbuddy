...
import { cn } from "@/lib/utils"; // formatCurrency tidak dipakai lagi, jadi dihapus dr import kalau mau, atau biarin aja
...
export default async function RoomsListPage() {
  ...
  // 2. CEK STATUS ACTIVE/DONE
  const roomsWithStatus = await Promise.all(
    (myRooms || []).map(async (item: any) => {
      const room = item.room;

      // Kita tetap ambil activities cuma buat ngecek status LUNAS/BELUM (bukan buat nominal)
              ...
              <Link key={room.id} href={`/dashboard/rooms/${room.id}`}>
                ...
                >
                  ...
                  <CardContent>
                    ...
                    {/* BAGIAN TOTAL PENGELUARAN SUDAH SAYA HAPUS DI SINI BIAR BERSIH */}
                  </CardContent>
